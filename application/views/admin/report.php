  ...
  <div class="row pb-4">
    <div class="col">
      <form action="" method="post">
        ...
        <input type="date" name="from" id="from" class="p-2 rounded border border-secondary">

        <label class="mt-4 mx-4 font-weight-bold" for="to">to</label>
        <input type="date" name="to" id="to" class="p-2 rounded border border-secondary">
        ...
        <button type="button" class="btn btn px-4" onclick="printdocument('content');">print</button>
      </form>
    </div>
  </div>
      ...
      <table class="table border shadow-sm mt-3">
        ...
        <?php if(isset($_POST['btnSearch'])) { 
          ...
          <tr>
            ...
            <td><?php echo date('d F Y', strtotime($data['time'])); ?></td>
          </tr>
        <?php endforeach; } ?>
        </tbody>
      </table>
